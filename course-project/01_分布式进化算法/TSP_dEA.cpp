// Algorithm 1: 分布式进化算法 (Distributed Evolutionary Algorithm)
// Master-worker model: rank 0 broadcasts the full population each generation,
// each worker mutates a chunk, results are gathered back, master selects.
// Supports multiple runs with result logging for statistical analysis.
// Compile: cl /EHsc /I"%MSMPI_INC%" TSP_dEA.cpp /link /LIBPATH:"%MSMPI_LIB64%" msmpi.lib
// Run: mpiexec -n 4 TSP_dEA.exe
...
void printResult(long gen, double best, double elapsed, FILE *fp);
void select1();
...
int main(int argc, char **argv)
{
    ...
    // Division of work among processes
    ...
    if (rank == 0)
    {
        ...
        if (resFile)
        {
            fprintf(resFile, "# Algorithm 1: dEA (Distributed Evolutionary Algorithm)\n");
            fprintf(resFile, "# pcb442.tsp, population=%d, maxGen=%ld, processes=%d\n", xColony, maxGen, size);
    ...
    for (int run = 0; run < numRuns; run++)
    {
        ...
        {
            int array[CITY];
            for (int i = 0; i < xCity; i++) array[i] = i;
            ...
            if (run == 0) printf("init success! initial best = %f\n", sumbest);
        }
